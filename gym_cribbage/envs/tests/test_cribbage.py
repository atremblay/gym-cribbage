...
from cribbage_env import is_sequence, Card, RANKS, SUITS, Stack, evaluate_hand, CribbageEnv, card_to_idx, stack_to_idx
...
class CribbageEnvTest(unittest.TestCase):
    ...
    def test_evaluate_hand(self):
        ...
        # extra point
        self.assertEqual(evaluate_hand(hand, knob=Card(RANKS[7], SUITS[0])), 9)
        self.assertEqual(evaluate_hand(hand, knob=Card(RANKS[7], SUITS[1])), 8)

        # Crib hand needs to have all same suit including the knob, otherwise
        # you don't get the points for the same suit
        self.assertEqual(evaluate_hand(hand, knob=Card(RANKS[7], SUITS[0]), is_crib=True), 9)
        self.assertEqual(evaluate_hand(hand, knob=Card(RANKS[7], SUITS[1]), is_crib=True), 4)

        hand = Stack(cards=[Card(RANKS[i], SUITS[0]) for i in range(5)])
        self.assertEqual(evaluate_hand(hand), 12)

        hand = Stack(cards=[Card(RANKS[i], SUITS[0]) for i in range(4)])
        knob = Card(RANKS[4], SUITS[0])
        self.assertEqual(evaluate_hand(hand, knob), 12)

        hand = Stack(cards=[Card(RANKS[i], SUITS[0]) for i in range(4)])
        knob = Card(RANKS[5], SUITS[0])
        self.assertEqual(evaluate_hand(hand, knob), 11)

        hand = Stack(cards=[Card(RANKS[i], SUITS[0]) for i in [1, 1, 2, 3, 4]])
        self.assertEqual(evaluate_hand(hand), 15)

        hand = Stack(cards=[Card(RANKS[i], SUITS[0]) for i in [1, 1, 1, 2, 3]])
        self.assertEqual(evaluate_hand(hand), 20)

        hand = Stack(cards=[Card(RANKS[i], SUITS[0]) for i in [1, 3, 5, 7]])
        knob = Card(RANKS[9], SUITS[0])
        self.assertEqual(evaluate_hand(hand, knob), 5)

        hand = Stack(cards=[Card(RANKS[i], SUITS[0]) for i in [1, 3, 5, 7]])
        knob = Card(RANKS[9], SUITS[1])
        self.assertEqual(evaluate_hand(hand, knob), 4)

        hand = Stack(cards=[Card(RANKS[i], SUITS[0]) for i in [1, 3, 5, 10]])
        self.assertEqual(evaluate_hand(hand, knob=Card(RANKS[-1], SUITS[0])), 6)
        ...
        self.assertEqual(evaluate_hand(hand, knob=Card(RANKS[4], SUITS[3])), 29)
